...
def affiche_graphique(symbol, period, short_period, long_period, signal_period):
    
    def money_flow_index(symbol, period):
        data1 = transformation(symbol)
        flow_positive = []
        flow_negative = []
    ...
    def moving_average_convergence_divergence(symbol, short_period, long_period, signal_period):
        df = transformation(symbol)
    ...
    axes[0].plot(money_flow_index(symbol, period)[1:], color='blue', label='MFI Line') #[1:] pour ne pas commencer à 0 à cause du .shift(1)
    ...
    axes[1].plot(relative_strength_index(symbol, period), color='blue', label='RSI Line')
    ...
    macd_line, signal_line, histogram = moving_average_convergence_divergence(symbol, short_period, long_period, signal_period)
...
if __name__=='__main__':
    ...
    #print(data)
    print(stats(symbol)[0])
    print(stats(symbol)[1])
    print(stats(symbol)[2])
    print(stats(symbol)[3])
    print(stats(symbol)[4])
    print(stats(symbol)[5])
    print(stats(symbol)[6])

    affiche_graphique(symbol, 14, short_period, long_period, signal_period)
